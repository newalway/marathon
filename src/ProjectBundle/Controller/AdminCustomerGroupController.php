...
use ProjectBundle\Form\Type\AdminCustomerGroupType;
...
{
	const ROUTER_INDEX = 'admin_customer_group';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
	const ROUTER_CONTROLLER = 'AdminCustomerGroup';

	protected function getQuerySearchData($arr_query_data)
	{
		$repository = $this->getDoctrine()->getRepository(CustomerGroup::class);
	...
	*/
	public function newAction(Request $request)
	{
		...
		$form = $this->createForm(AdminCustomerGroupType::class, new CustomerGroup());
	...
	*/
	public function createAction(Request $request)
	{
		...
		$form = $this->createForm(AdminCustomerGroupType::class, $data);
	...
	*/
	public function editAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$util->setCkAuthorized();
		$data = $this->getDoctrine()->getRepository(CustomerGroup::class)->find($request->get('id'));
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(CustomerGroup::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
		...
		if ($form->isSubmitted() && $form->isValid()) {
			...
			if($request->get('removefileicon')==1){
				$data->removeIcon();
			}
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(CustomerGroup::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
						$em->flush();
