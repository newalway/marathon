...
use ProjectBundle\Form\Type\AdminEquipmentType;
...
{
	const ROUTER_INDEX = 'admin_equipment';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
  const ROUTER_PREFIX = 'equipment';
	const ROUTER_CONTROLLER = 'AdminEquipment';
  ...
  */
  public function newAction(Request $request)
  {
		...
		$form = $this->createForm(AdminEquipmentType::class, new Equipment());
  ...
  */
  public function createAction(Request $request)
  {
    ...
    $em = $this->getDoctrine()->getManager();
		...
		$data = new Equipment();
    $form = $this->createForm(AdminEquipmentType::class, $data);
  ...
  */
  public function editAction(Request $request)
  {
    $util = $this->container->get('utilities');
		$util->setCkAuthorized();
		$data = $this->getDoctrine()->getRepository(Equipment::class)->find($request->get('id'));
  ...
  */
  public function updateAction(Request $request)
  {
    ...
    $em = $this->getDoctrine()->getManager();
		...
		$data = $em->getRepository(Equipment::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
  ...
  public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(Equipment::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
				    $em->flush();
