...
use ProjectBundle\Form\Type\AdminLayoutShopType;
...
class AdminLayoutShopController extends Controller
{
	const ROUTER_INDEX = 'admin_layout_shop';
	const ROUTER_ADD = self::ROUTER_INDEX.'_new';
	const ROUTER_EDIT = self::ROUTER_INDEX.'_edit';
	const ROUTER_CONTROLLER = 'AdminLayoutShop';

	protected function getQuerySearchData($arr_query_data,$locale)
	{
		$repository = $this->getDoctrine()->getRepository(LayoutShop::class);
    	$query = $repository->findAllData($arr_query_data, $locale);
		return $query;
	}
	...
	*/
	public function newAction(Request $request)
	{
		...
		$form = $this->createForm(AdminLayoutShopType::class, new LayoutShop());
	...
	*/
	public function createAction(Request $request)
	{
		...
		$data = new LayoutShop();
		$form = $this->createForm(AdminLayoutShopType::class, $data);
	...
	*/
	public function editAction(Request $request)
	{
		$util = $this->container->get('utilities');
		$util->setCkAuthorized();
		$data = $this->getDoctrine()->getRepository(LayoutShop::class)->find($request->get('id'));
	...
	*/
	public function updateAction(Request $request)
	{
		...
		$data = $em->getRepository(LayoutShop::class)->find($request->get('id'));
		if (!$data) { throw $this->createNotFoundException('No data found'); }
	...
	public function group_deleteAction(Request $request)
	{
		...
		if ($data_ids) {
			...
			foreach ($data_ids as $data_id) {
				$data = $em->getRepository(LayoutShop::class)->find($data_id);
				if ($data) {
					try {
						$em->remove($data);
						$em->flush();
